  ...
  return (
    <main className="m5e-screen" aria-labelledby={'m5e-title-' + spec.number}>
      <article className="m5e-shell">
        ...
        <section className="m5e-saved" role="status" aria-live="polite"><strong>Saved output</strong><span>{spec.output}: {canContinue ? 'ready to carry forward' : 'in progress'}. Your work saves automatically.</span></section>
...
function CanvasScreen({ state, onChangeState }: Omit<Props, 'screenId'>) {
  ...
  const update = (id: string, value: string) => {
    ...
    setFields(next); setConfirmedSafe(false); setPreviewReviewed(false); setMessage('Changes saved. Review the preview and safety confirmation again.');
  ...
  return (
    <main className="m5e-screen" aria-labelledby="m5e-complete-title">
      <article className="m5e-shell">
        ...
        <section className="m5e-plan" aria-labelledby="m5e-plan-title"><h2 id="m5e-plan-title">90-Day Learning and Account-Back Plan</h2>{planFields.map(([id, label, help]) => { const fieldRisky = containsPotentiallySensitiveModule5Text(plan[id] || ''); const errorId = 'm5e-plan-error-' + id; return <label key={id}><span><strong>{label}</strong><small>{help}</small></span><textarea rows={3} maxLength={320} value={plan[id] || ''} aria-invalid={fieldRisky || undefined} aria-describedby={fieldRisky ? errorId : undefined} onChange={(event) => { const next = { ...plan, [id]: event.target.value }; setPlan(next); setConfirmedSafe(false); setMessage(containsPotentiallySensitiveModule5Text(event.target.value) ? 'Remove possible identifying or sensitive detail from the highlighted field. This prompt is a precaution and cannot guarantee confidentiality.' : 'Changes saved. Review the safety confirmation again.'); persist(next, dashboardReviewed, carryReviewed, false); }} />{fieldRisky && <small id={errorId} className="m5e-field-error">Remove possible identifying or sensitive detail.</small>}</label>; })}</section>
