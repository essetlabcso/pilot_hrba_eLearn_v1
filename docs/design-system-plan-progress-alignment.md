...
| Full platform/course audit | Identify infrastructure, legacy code, candidate migration areas, and design debt. | `docs/token-adoption-audit.md`, `docs/player-shell-state-modal-token-inventory.md`, `docs/player-shell-state-recipe-decision.md`, `docs/player-shell-interaction-state-recipe-specification.md`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, `docs/player-shell-hover-on-dark-token-category-specification.md`, `docs/player-shell-hover-on-dark-token-value-proposal.md`, `docs/player-shell-hover-on-dark-token-file-qa.md`, and earlier repo audit work identify style entry points and risks. | Partial | Audit is still visual/token focused; broader component inventory remains. | Medium | If approved, create a separate bounded CSS hover implementation task; do not combine it with broader migration. |
...
| Token foundation | Create approved token values and implementation-ready spec. | `docs/implementation-ready-token-specification.md`, `src/system/tokens/tokens.ts`, `src/system/tokens/tokens.css`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, `docs/player-shell-hover-on-dark-token-category-specification.md`, `docs/player-shell-hover-on-dark-token-value-proposal.md`, and `docs/player-shell-hover-on-dark-token-file-qa.md`. | Partial | Hover-on-dark token variables are implemented and QA passed, but tokens are not fully adopted across the app and automated prevention is not in place. | Medium | Keep CSS hover implementation blocked until a separate bounded CSS task is approved. |
| Player shell token adoption | Bring player shell surfaces, text, icons, base buttons, and bounded focus into token control. | Token aliasing, cascade fix, surface/border adoption, text/icon adoption, button base adoption, state/modal inventory, focus-visible adoption, state recipe decision, interaction-state recipe specification, hover-on-dark token/alias decision, hover-on-dark token category specification, hover-on-dark token value proposal, hover-on-dark token-file implementation, token-file QA, and earlier QA notes. | In progress | Hover token variables exist, but no CSS selector usage exists yet. Hover, active, disabled, selected, completed, locked, modal, accessibility UI, danger-on-dark, progress strip, and shell shadows remain legacy or undecided. | Medium | Keep implementation paused until a separate bounded CSS hover task is explicitly approved. |
| Interaction state recipes | Define and apply state rules for hover, active, selected, completed, locked, disabled, error, success, loading. | Component state requirements exist in token specification; `docs/player-shell-state-modal-token-inventory.md` identifies state groups; `docs/player-shell-state-recipe-decision.md` pauses implementation; `docs/player-shell-interaction-state-recipe-specification.md` defines draft player shell state recipes and gaps; `docs/player-shell-hover-on-dark-token-alias-decision.md` rejects hover aliases; `docs/player-shell-hover-on-dark-token-category-specification.md` defines semantic hover categories; `docs/player-shell-hover-on-dark-token-value-proposal.md` proposes values; `docs/player-shell-hover-on-dark-token-file-qa.md` verifies token files. | Partial | Hover token variables exist, but no state selector implementation is approved. CSS hover implementation remains blocked. | High | If approved, create a separate bounded CSS hover implementation task for non-danger hover only; do not broaden state migration. |
...
| Player shell interaction-state recipe specification | Created `docs/player-shell-interaction-state-recipe-specification.md`. | Defines player shell state semantics, selector boundaries, token category mapping, accessibility rules, QA checks, and stop conditions for future state work. | Does not approve implementation. The first likely future candidate, non-danger hover-on-dark, remains blocked until hover-on-dark surface/border token categories or an approved alias strategy are reviewed. | Interaction state recipes and visual-token stream |
| Player shell hover-on-dark token/alias decision | Created `docs/player-shell-hover-on-dark-token-alias-decision.md`. | Records that existing inverse surface/border tokens must not be reused as hover aliases because they are structural/base tokens and may be invisible or too subtle on dark sidebar surfaces. | Does not implement hover. Non-danger hover-on-dark remains blocked until semantic hover-on-dark surface/border categories and high-contrast mapping are documented. | Interaction state recipes, token foundation, and visual-token stream |
| Player shell hover-on-dark token category specification | Created `docs/player-shell-hover-on-dark-token-category-specification.md`. | Defines semantic categories for player shell hover-on-dark surface, border, optional text/icon, and high-contrast hover surface/border mapping. | Does not assign values, create token variables, edit token files, or approve CSS implementation. Non-danger hover-on-dark remains blocked until values and token files are handled in separate approved tasks. | Token foundation, interaction state recipes, and visual-token stream |
| Player shell hover-on-dark token value proposal | Created `docs/player-shell-hover-on-dark-token-value-proposal.md`. | Recommends conservative candidate values for review: `--cso-color-state-hover-on-inverse-surface: #253449`, `--cso-color-state-hover-on-inverse-border: #64748B`, `--cso-color-high-contrast-hover-surface: #1A1A1A`, and `--cso-color-high-contrast-hover-border: #7DD3FC`. It also recommends no separate hover text/icon token for the first slice. | Does not approve implementation, edit token files, create token variables, or implement CSS. Non-danger hover-on-dark CSS remains blocked until token files are updated in a separate approved task. | Token foundation, interaction state recipes, and visual-token stream |
| Player shell hover-on-dark token-file implementation and QA | Implemented hover-on-dark token variables in `src/system/tokens/tokens.css` and matching entries in `src/system/tokens/tokens.ts`; QA documented in `docs/player-shell-hover-on-dark-token-file-qa.md`. | Verifies implementation commit `7679dca5e8be70b212d5c6c3277c0ddd09887f20`, QA commit `5768e37a313fb9f044c063a445bd7631b974388f`, exact CSS variables, matching TypeScript entries, `npm run build` pass with only the existing Vite large-chunk warning, no selector usage, and unchanged `src/styles/global.css`. | Does not implement CSS hover selectors, apply tokens to UI, change React behavior, or migrate active/current, disabled, danger, modal/accessibility UI, progress strip, shell shadow, global focus, course-screen, or module-specific states. CSS hover implementation remains blocked until a separate bounded CSS task is approved. | Token foundation, interaction state recipes, visual-token stream, and QA evidence |
...
| --- | --- | --- |
| Visual token/player-shell adoption stream | About 51 percent | Token files, CSS wiring, shell aliases, surfaces, text/icons, base buttons, Phase B state/modal inventory, bounded player shell focus-visible adoption, interaction-state recipe specification, hover-on-dark token/alias decision, hover-on-dark token category specification, hover-on-dark token value proposal, hover-on-dark token-file implementation, token-file QA, and route QA are done. Implementation remains paused because CSS hover application, modals, prevention gates, global/course focus, and broader shell QA remain. |
...
| Token tunnel vision | Work may continue replacing colors while ignoring components, states, templates, and learning design. | High | Token adoption audit, Phase B/Phase C evidence, state recipe decision, interaction-state recipe specification, hover-on-dark token/alias decision, hover category specification, and token-file QA recommend bounded migration order. | Keep implementation paused and require a separate bounded CSS task before any selector usage. |
| Hover-on-dark implementation gap | Hover-on-dark token variables are implemented and QA passed, but no CSS selectors use them yet. | Medium | `docs/player-shell-hover-on-dark-token-file-qa.md` verifies exact variables, matching TypeScript entries, build pass, no selector usage, and unchanged `src/styles/global.css`. | CSS implementation remains blocked until a separate bounded CSS hover task is approved. |
| Accessibility states still legacy | Player shell focus-visible passed QA, but global focus, course-screen focus, modal focus, disabled, selected/current, locked, completed, active, danger, and other state styles still include legacy styling. | Medium-high | Token spec defines state requirements; Phase C QA confirms the bounded player shell focus slice; interaction-state recipe specification documents remaining state gaps. | Keep broader accessibility state risk open until a documented token/alias decision, implementation slice, and QA pass for the next state area. |
...
| Hard-coded visual values may continue | Manual review can miss new raw values. | High | Manual diff checks have been used; hover token-file QA confirmed no selector usage was added. | Add hard-coded visual value prevention phase; this remains unresolved. |
...
Create a separate bounded CSS hover implementation task only if explicitly approved.

CSS hover implementation remains blocked until that separate bounded task is approved.

The future task should apply only the already implemented hover-on-dark token variables to non-danger player shell hover selectors.

It should cover only:

- non-danger player shell hover selectors;
- use of `--cso-color-state-hover-on-inverse-surface`;
- use of `--cso-color-state-hover-on-inverse-border`;
- preserving existing inverse text/icon tokens;
- preserving Phase C focus-visible behavior;
- desktop, tablet, and mobile QA on the approved player route;
- confirmation that no active/current, disabled, danger, modal/accessibility UI, progress strip, shell shadow, global focus, course-screen, or module-specific styles were migrated.

Do not combine the future CSS hover task with any other state migration.
...
No React, routing, progress, assessment, accessibility toolbar, modal behavior, asset, or old course file should be changed during the future bounded CSS hover task.
