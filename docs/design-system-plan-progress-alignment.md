...
| Full platform/course audit | Identify infrastructure, legacy code, candidate migration areas, and design debt. | `docs/token-adoption-audit.md`, `docs/player-shell-state-modal-token-inventory.md`, `docs/player-shell-state-recipe-decision.md`, `docs/player-shell-interaction-state-recipe-specification.md`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, `docs/player-shell-hover-on-dark-token-category-specification.md`, `docs/player-shell-hover-on-dark-token-value-proposal.md`, and earlier repo audit work identify style entry points and risks. | Partial | Audit is still visual/token focused; broader component inventory remains. | Medium | If the value proposal is accepted, create a bounded token-file implementation task for approved hover-on-dark tokens only. |
...
| Token foundation | Create approved token values and implementation-ready spec. | `docs/implementation-ready-token-specification.md`, `src/system/tokens/tokens.ts`, `src/system/tokens/tokens.css`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, `docs/player-shell-hover-on-dark-token-category-specification.md`, and `docs/player-shell-hover-on-dark-token-value-proposal.md`. | Partial | Tokens exist, but not fully adopted and not backed by automated prevention. Hover-on-dark values are proposed for review, but token files and token variables are not yet implemented. | Medium | If accepted, create a bounded token-file implementation task for approved hover-on-dark tokens only; do not implement CSS yet. |
| Player shell token adoption | Bring player shell surfaces, text, icons, base buttons, and bounded focus into token control. | Token aliasing, cascade fix, surface/border adoption, text/icon adoption, button base adoption, state/modal inventory, focus-visible adoption, state recipe decision, interaction-state recipe specification, hover-on-dark token/alias decision, hover-on-dark token category specification, hover-on-dark token value proposal, and QA notes. | In progress | Hover, active, disabled, selected, completed, locked, modal, accessibility UI, danger-on-dark, progress strip, and shell shadows remain legacy or undecided. First likely future candidate is non-danger hover-on-dark, but CSS implementation remains blocked until token files are approved and updated in a later task. | Medium | Keep implementation paused; if values are accepted, next create a bounded token-file task only. |
| Interaction state recipes | Define and apply state rules for hover, active, selected, completed, locked, disabled, error, success, loading. | Component state requirements exist in token specification; `docs/player-shell-state-modal-token-inventory.md` identifies state groups; `docs/player-shell-state-recipe-decision.md` pauses implementation; `docs/player-shell-interaction-state-recipe-specification.md` defines draft player shell state recipes and gaps; `docs/player-shell-hover-on-dark-token-alias-decision.md` rejects hover aliases; `docs/player-shell-hover-on-dark-token-category-specification.md` defines semantic hover categories; `docs/player-shell-hover-on-dark-token-value-proposal.md` proposes values. | Partial | Recipes/categories/proposed values exist, but no state implementation is approved. Token variables are not yet implemented and CSS remains blocked. | High | If accepted, create a bounded token-file implementation task for hover-on-dark tokens only; do not implement state CSS yet. |
...
| --- | --- | --- |
| Visual token/player-shell adoption stream | About 50 percent | Token files, CSS wiring, shell aliases, surfaces, text/icons, base buttons, Phase B state/modal inventory, bounded player shell focus-visible adoption, interaction-state recipe specification, hover-on-dark token/alias decision, hover-on-dark token category specification, hover-on-dark token value proposal, and route QA are done. Implementation remains paused because hover-on-dark token files, modals, prevention gates, global/course focus, and broader shell QA remain. |
...
| Token tunnel vision | Work may continue replacing colors while ignoring components, states, templates, and learning design. | High | Token adoption audit, Phase B/Phase C evidence, state recipe decision, interaction-state recipe specification, hover-on-dark token/alias decision, and hover category specification recommend bounded migration order. | Keep implementation paused and use documentation gates for the hover-on-dark value proposal before any token-file or CSS work. |
| Hover-on-dark value gap | Proposed hover-on-dark values exist, but they are not yet accepted or implemented as token variables. | Medium | `docs/player-shell-hover-on-dark-token-value-proposal.md` recommends a conservative value set and keeps CSS implementation blocked. | If accepted, create a bounded token-file implementation task for approved hover-on-dark tokens only; do not implement CSS yet. |
...
If the hover-on-dark value proposal is accepted, create a bounded token-file implementation task for approved hover-on-dark tokens only.

This must be token-file-only unless a future prompt explicitly approves documentation updates in the same scope.

The task should add only the accepted semantic hover-on-dark token variables and matching TypeScript token entries.
...
- matching TypeScript token object entries;
- token-file build validation;
- confirmation that no CSS selectors use the new tokens yet.

Do not implement CSS hover behavior in that step.
...
No CSS, React, routing, progress, assessment, accessibility toolbar, modal behavior, asset, or old course file should be changed during the bounded token-file implementation task.
