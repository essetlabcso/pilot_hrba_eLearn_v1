...
Create a documentation-only modal/drawer behavior review or, if preferred, a token/CSS readiness check before any current-state CSS task.
...
Phase D CSS remains blocked. Modal launcher and menu drawer ARIA remain blocked pending separate modal/drawer behavior review.
...
No CSS, token files, routing, progress, assessment, accessibility toolbar behavior, modal behavior, asset, or old course file should be changed during the future modal/drawer behavior review or token/CSS readiness check unless that task is separately approved and explicitly scoped.
