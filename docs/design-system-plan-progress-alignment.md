...
Human-review `docs/player-shell-current-on-inverse-token-value-proposal.md`.
...
It proposes review-only candidate values:
...
If these values are accepted, the next implementation step should be a bounded token-file-only task for current-on-inverse tokens. That task must not implement CSS selectors or React behavior.

Any future CSS implementation task after token files, React/accessibility implementation specification, and any required modal behavior review must identify one state area, one selector family, one validation route, and one QA note. It must not treat the completed hover slice, readiness note, category decision, semantics review, or value proposal as approval for broader state migration.
