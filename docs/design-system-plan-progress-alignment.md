...
- modal and accessibility UI beyond the bounded Glossary, Resources, and Accessibility focus return/containment slice;
...
If approved, create a separate bounded Slice 2 implementation task from `docs/player-shell-modal-root-id-launcher-aria-readiness.md`: add stable modal root IDs and launcher ARIA for Glossary, Resources, and Accessibility only.
...
The Slice 2 root ID and launcher ARIA readiness note is complete in `docs/player-shell-modal-root-id-launcher-aria-readiness.md`.

It recommends:

- future modal root IDs: `player-glossary-modal`, `player-resources-modal`, and `player-accessibility-modal`;
- preserving existing title IDs: `glossary-modal-title`, `resources-modal-title`, and `a11y-modal-title`;
- using `aria-expanded` for the three scoped launchers;
- using conditional `aria-controls` only while each conditionally rendered modal is mounted;
- using `aria-haspopup="dialog"` for those three scoped modal launchers;
- keeping Menu drawer, HelpOverlay, Captions/transcript, CSS, token work, routing, progress, assessment, certificate logic, accessibility toolbar behavior, content, assets, course screens, module CSS, and old HRBA files out of scope.
...
Phase D CSS remains blocked. Modal launcher and Menu drawer ARIA remain blocked pending a bounded modal/drawer behavior implementation specification and separately approved implementation tasks.
...
It defines future implementation slices. The first slice is now complete:
...
- Slice 2: stable modal root IDs and launcher ARIA preparation for those three modals - readiness complete, implementation not started;
...
Phase D CSS remains blocked. Launcher ARIA implementation remains blocked until the Slice 2 readiness note is reviewed and a separate bounded implementation task is approved.
...
No CSS, token files, routing, progress, assessment, accessibility toolbar behavior, asset, content, or old course file should be changed during any future Slice 2 implementation task.
