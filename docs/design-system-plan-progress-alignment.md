...
Create a documentation-only active/current sidebar accessibility and behavior semantics review before any current-on-inverse token value proposal or Phase D CSS migration.
...
It also records that active/current CSS implementation remains blocked because the accessible state pattern for active sidebar tool buttons and transcript visibility is unresolved. The next documentation-only review should decide whether these controls should expose state through `aria-pressed`, `aria-expanded`, or another appropriate pattern. `aria-current` is not recommended for the sidebar tool buttons because they are not page or route navigation items.

It must not implement CSS, token values, token files, or React behavior.

Any future implementation task after the semantics review and any required token value decision must identify one state area, one selector family, one validation route, and one QA note. It must not treat the completed hover slice, readiness note, or category decision as approval for broader state migration.
