...
| Full platform/course audit | Identify infrastructure, legacy code, candidate migration areas, and design debt. | `docs/token-adoption-audit.md`, `docs/player-shell-state-modal-token-inventory.md`, `docs/player-shell-state-recipe-decision.md`, `docs/player-shell-interaction-state-recipe-specification.md`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, and earlier repo audit work identify style entry points and risks. | Partial | Audit is still visual/token focused; broader component inventory remains. | Medium | Use the hover-on-dark decision to create a documentation-only semantic hover token/category update. |
...
| Token foundation | Create approved token values and implementation-ready spec. | `docs/implementation-ready-token-specification.md`, `src/system/tokens/tokens.ts`, `src/system/tokens/tokens.css`, and `docs/player-shell-hover-on-dark-token-alias-decision.md`. | Partial | Tokens exist, but not fully adopted and not backed by automated prevention. The hover-on-dark decision rejects structural/base-token aliases and requires semantic hover-on-dark surface/border categories before CSS migration. | Medium | Create a documentation-only token specification/category update for semantic hover-on-dark surface and border tokens; do not edit token files or CSS. |
| Player shell token adoption | Bring player shell surfaces, text, icons, base buttons, and bounded focus into token control. | Token aliasing, cascade fix, surface/border adoption, text/icon adoption, button base adoption, state/modal inventory, focus-visible adoption, state recipe decision, interaction-state recipe specification, hover-on-dark token/alias decision, and QA notes. | In progress | Hover, active, disabled, selected, completed, locked, modal, accessibility UI, danger-on-dark, progress strip, and shell shadows remain legacy or undecided. First likely future candidate is non-danger hover-on-dark, but it remains blocked because semantic hover-on-dark token categories are required first. | Medium | Keep implementation paused; next create a documentation-only semantic hover token/category update, not CSS. |
| Interaction state recipes | Define and apply state rules for hover, active, selected, completed, locked, disabled, error, success, loading. | Component state requirements exist in token specification; `docs/player-shell-state-modal-token-inventory.md` identifies state groups; `docs/player-shell-state-recipe-decision.md` pauses implementation; `docs/player-shell-interaction-state-recipe-specification.md` defines draft player shell state recipes and gaps; `docs/player-shell-hover-on-dark-token-alias-decision.md` rejects hover aliases. | Partial | Recipe specification and hover decision exist, but no state implementation is approved. Semantic hover-on-dark surface/border categories and high-contrast mapping are still required. | High | Create documentation-only semantic hover token/category update; do not implement state CSS yet. |
...
| --- | --- | --- |
| Visual token/player-shell adoption stream | About 50 percent | Token files, CSS wiring, shell aliases, surfaces, text/icons, base buttons, Phase B state/modal inventory, bounded player shell focus-visible adoption, interaction-state recipe specification, hover-on-dark token/alias decision, and route QA are done. Implementation remains paused because semantic hover-on-dark token categories, modals, prevention gates, global/course focus, and broader shell QA remain. |
...
| Token tunnel vision | Work may continue replacing colors while ignoring components, states, templates, and learning design. | High | Token adoption audit, Phase B/Phase C evidence, state recipe decision, interaction-state recipe specification, and hover-on-dark token/alias decision recommend bounded migration order. | Keep implementation paused until semantic hover-on-dark token categories are documented and reviewed. |
...
Create a documentation-only semantic hover-on-dark token specification/category update.

This must be documentation-only.

The update should define semantic token categories for non-danger hover-on-dark before any CSS implementation.

It should cover only:

- hover-on-dark surface category;
- hover-on-dark border category;
- hover-on-dark high contrast surface and border mapping;
...
No CSS, React, token file, routing, progress, assessment, accessibility toolbar, modal behavior, asset, or old course file should be changed during the semantic hover-on-dark token specification/category update.
