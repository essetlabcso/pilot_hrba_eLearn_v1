...
If approved, implement the bounded Captions/transcript React/accessibility semantics slice only.
...
The React/accessibility implementation specification exists at `docs/player-shell-active-current-sidebar-react-accessibility-implementation-spec.md`.

It recommends the smallest future implementation slice:
...
This future implementation must be separate from CSS and must not touch token files, modal launchers, menu drawer, routing, progress, assessment, certificate logic, accessibility toolbar behavior, assets, content, or old HRBA files.

Any future CSS implementation task after token files, React/accessibility implementation, and any required modal behavior review must identify one state area, one selector family, one validation route, and one QA note. It must not treat the completed hover slice, readiness note, category decision, semantics review, value proposal, token-file QA, or React/accessibility spec as approval for broader state migration.
...
No CSS, token files, routing, progress, assessment, accessibility toolbar behavior, modal behavior, asset, or old course file should be changed during the future Captions/transcript React/accessibility slice unless that task is separately approved and explicitly scoped.
