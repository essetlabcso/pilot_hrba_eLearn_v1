...
Create a documentation-only Phase D navigation-state implementation readiness note before any CSS migration.
...
The next safe step should inspect and plan active/current/selected/completed/locked/disabled player shell navigation states before any CSS changes.

The Phase D readiness note should identify:

- affected player shell navigation selectors;
- current active/current/selected/completed/locked/disabled visual patterns;
- learner meaning and accessibility role for each state;
- token/spec gaps;
- behavior risks;
- smallest possible future implementation slice;
- QA requirements;
- stop conditions.

It must not implement CSS.

Any future implementation task after that readiness note must identify one state area, one selector family, one validation route, and one QA note. It must not treat the completed hover slice as approval for broader state migration.
