...
If approved, implement Slice 1 only from `docs/player-shell-modal-drawer-behavior-implementation-spec.md`: shared modal focus return and focus containment for Glossary, Resources, and Accessibility.
...
- Glossary, Resources, and Accessibility have partial dialog semantics, Escape close, outside-click close, close controls, and initial focus movement, but still need focus return and focus containment decisions;
...
It defines future implementation slices and recommends the first future implementation candidate:

- Slice 1: shared modal focus return and focus containment for Glossary, Resources, and Accessibility only;
- Slice 2: stable modal root IDs and launcher ARIA preparation for those three modals;
...
Phase D CSS remains blocked. Launcher ARIA remains blocked until behavior implementation and QA pass.
...
- modal launcher React/accessibility implementation;
...
No CSS, token files, routing, progress, assessment, accessibility toolbar behavior, asset, content, or old course file should be changed during the future Slice 1 modal behavior implementation unless that task is separately approved and explicitly scoped.
