...
| Full platform/course audit | Identify infrastructure, legacy code, candidate migration areas, and design debt. | `docs/token-adoption-audit.md`, `docs/player-shell-state-modal-token-inventory.md`, `docs/player-shell-state-recipe-decision.md`, `docs/player-shell-interaction-state-recipe-specification.md`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, `docs/player-shell-hover-on-dark-token-category-specification.md`, and earlier repo audit work identify style entry points and risks. | Partial | Audit is still visual/token focused; broader component inventory remains. | Medium | Use the hover-on-dark category specification to create a documentation-only token value proposal or update to `docs/implementation-ready-token-specification.md`. |
...
| Token foundation | Create approved token values and implementation-ready spec. | `docs/implementation-ready-token-specification.md`, `src/system/tokens/tokens.ts`, `src/system/tokens/tokens.css`, `docs/player-shell-hover-on-dark-token-alias-decision.md`, and `docs/player-shell-hover-on-dark-token-category-specification.md`. | Partial | Tokens exist, but not fully adopted and not backed by automated prevention. Semantic hover-on-dark categories are specified, but final values, high-contrast values, and token variables are not created. | Medium | Create a documentation-only hover-on-dark token value proposal or update to `docs/implementation-ready-token-specification.md`; do not edit token files or CSS unless separately approved. |
| Player shell token adoption | Bring player shell surfaces, text, icons, base buttons, and bounded focus into token control. | Token aliasing, cascade fix, surface/border adoption, text/icon adoption, button base adoption, state/modal inventory, focus-visible adoption, state recipe decision, interaction-state recipe specification, hover-on-dark token/alias decision, hover-on-dark token category specification, and QA notes. | In progress | Hover, active, disabled, selected, completed, locked, modal, accessibility UI, danger-on-dark, progress strip, and shell shadows remain legacy or undecided. First likely future candidate is non-danger hover-on-dark, but CSS implementation remains blocked until values and token files are approved in later tasks. | Medium | Keep implementation paused; next create a documentation-only hover-on-dark token value proposal, not CSS. |
| Interaction state recipes | Define and apply state rules for hover, active, selected, completed, locked, disabled, error, success, loading. | Component state requirements exist in token specification; `docs/player-shell-state-modal-token-inventory.md` identifies state groups; `docs/player-shell-state-recipe-decision.md` pauses implementation; `docs/player-shell-interaction-state-recipe-specification.md` defines draft player shell state recipes and gaps; `docs/player-shell-hover-on-dark-token-alias-decision.md` rejects hover aliases; `docs/player-shell-hover-on-dark-token-category-specification.md` defines semantic hover categories. | Partial | Recipes/categories exist, but no state implementation is approved. Semantic hover-on-dark values, high-contrast value mapping, and token-file implementation are still required. | High | Create documentation-only hover-on-dark token value proposal; do not implement state CSS yet. |
...
| --- | --- | --- |
| Visual token/player-shell adoption stream | About 50 percent | Token files, CSS wiring, shell aliases, surfaces, text/icons, base buttons, Phase B state/modal inventory, bounded player shell focus-visible adoption, interaction-state recipe specification, hover-on-dark token/alias decision, hover-on-dark token category specification, and route QA are done. Implementation remains paused because hover-on-dark values/token files, modals, prevention gates, global/course focus, and broader shell QA remain. |
...
| Hover-on-dark value gap | Semantic hover-on-dark categories exist, but approved values, high-contrast value mapping, and token variables do not. | Medium-high | `docs/player-shell-hover-on-dark-token-category-specification.md` defines the needed categories and keeps CSS implementation blocked. | Create a documentation-only hover-on-dark token value proposal or update to `docs/implementation-ready-token-specification.md`; do not edit token files or CSS unless separately approved. |
...
Create a documentation-only hover-on-dark token value proposal or documentation-only update to `docs/implementation-ready-token-specification.md`.

This must be documentation-only.

The proposal should define candidate values for the already-specified semantic non-danger hover-on-dark token categories before token files or CSS implementation.

It should cover only:

- proposed hover-on-dark surface value;
- proposed hover-on-dark border value;
- proposed high-contrast hover-on-dark surface and border values;
- whether hover text/icon tokens are needed or existing inverse text tokens should remain;
- how hover transform and reduced-motion rules should be treated in a future implementation slice;
...
Do not implement CSS, code, token files, or token values in that step.
...
No CSS, React, token file, routing, progress, assessment, accessibility toolbar, modal behavior, asset, or old course file should be changed during the documentation-only hover-on-dark token value proposal.
