...
Create a documentation-only modal accessibility behavior specification with Menu drawer behavior coverage before any modal launcher ARIA or current-state CSS task.
...
Phase D CSS remains blocked. Modal launcher and menu drawer ARIA remain blocked pending a separate modal accessibility behavior specification and any required Menu drawer behavior decisions.
...
No CSS, token files, routing, progress, assessment, accessibility toolbar behavior, modal behavior, asset, or old course file should be changed during the future modal accessibility behavior specification unless that task is separately approved and explicitly scoped.
