...
Create a documentation-only current-on-inverse token value proposal for the bounded active/current player sidebar tool state.
...
- a documentation-only current-on-inverse token value proposal can proceed next;
...
The next token value proposal must not implement CSS, token files, or React behavior.

Any future implementation task after the token value proposal, React/accessibility implementation specification, and any required modal behavior review must identify one state area, one selector family, one validation route, and one QA note. It must not treat the completed hover slice, readiness note, category decision, or semantics review as approval for broader state migration.
