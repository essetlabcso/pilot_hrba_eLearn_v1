...
Create a documentation-only semantic active/current-on-inverse token/category decision before any Phase D CSS migration.
...
It finds that Phase D CSS implementation is not ready yet. The smallest likely future implementation slice is bounded active/current sidebar tool state, but only after semantic current-state token categories and accessibility questions are resolved.

The next documentation-only token/category decision should cover:

- current-on-inverse surface;
- current-on-inverse border;
- current icon surface;
- high-contrast current-state mapping;
- relationship to focus-visible and hover-on-dark tokens;
- whether active sidebar tool buttons require accessible state semantics such as `aria-pressed`, `aria-expanded`, or another pattern.

It must not implement CSS.

Any future implementation task after that token/category decision must identify one state area, one selector family, one validation route, and one QA note. It must not treat the completed hover slice or the readiness note as approval for broader state migration.
