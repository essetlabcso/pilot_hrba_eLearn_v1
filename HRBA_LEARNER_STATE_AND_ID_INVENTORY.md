...
| --- | --- | --- |
| Closed release gate | HRBA `7f6bad9` passed matched Preview acceptance on 2026-07-25 while production remained unchanged. | Use `7f6bad9` as the update-development baseline; keep production release approval separate. |
| Closed integration gate | Hub contract `39dfa688` and HRBA `7f6bad9` passed authenticated two-learner same-browser, resume, assessment, completion, and certificate testing. | Preserve the accepted pair and rerun the same gate after contract-sensitive changes. |
| Draft PR release gate | Exact Hub commit `39dfa688` is accepted technically but remains in open Draft Hub PR #2 and is not in Hub `main`. | Do not merge or promote without the separate review and release process. |
| Closed baseline drift | The update parent is formally recorded as `7f6bad9`; production remains independently pinned to its prior deployment. | Keep Module 4/5 work on the accepted feature branch and retain the documented production rollback point. |
